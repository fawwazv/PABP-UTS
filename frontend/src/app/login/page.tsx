...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg("Login berhasil! Memuat dasbor...");
      
      setTimeout(() => {
        router.push("/");
      }, 1000); // Lebih cepat karena ke dashboard
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-md bg-[#0B1120]/60 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/10 p-10 relative z-10 flex flex-col items-center">
        ...
        <h2 className="text-3xl font-black mb-2 tracking-tight text-white drop-shadow-md">Selamat Datang</h2>
        <p className="text-slate-400 text-sm mb-8 text-center">Masuk ke akun Anda untuk memesan lapangan padel terbaik.</p>
        ...
        <form onSubmit={handleLogin} className="w-full flex flex-col gap-5">
          ...
          >
            {isLoading && !successMsg ? "Memproses..." : successMsg ? "Berhasil!" : "Masuk"}
            {!isLoading && !successMsg && <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />}
          </button>
        </form>

        <p className="mt-8 text-slate-400 text-sm">
          Belum punya akun? <Link href="/register" className="text-blue-400 font-bold hover:text-blue-300 transition border-b border-transparent hover:border-blue-300">Daftar Sekarang</Link>
        </p>
      </div>
    </div>
  );
